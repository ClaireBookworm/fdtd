...
def calculate_inc_dz_field(ia, ib, ja, jb, ka, kb, dz, hx_inc):
	...
	for i in range(ia, ib + 1):
		for k in range(ka, kb + 1):
			dz[i, ja, k] = dz[i, ja, k] + 0.5 * hx_inc[ja - 1] # equation is 
...
X, Y, Z = np.meshgrid(range(je), range(ie), range(ke))
...
# plot the E field in a 3d manner
...
ax.plot_surface(X, Y, ez[:, :, kc], rstride=1, cstride=1,
				color='white',
...
ax.text2D(0.6, 0.7, "T = {}".format(nsteps),
